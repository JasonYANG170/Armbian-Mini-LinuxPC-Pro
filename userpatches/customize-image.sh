...
apt-get update
apt-get install -y vim net-tools pv
apt-get install network-manager wpasupplicant wireless-tools iw

echo 'extraargs=rootflags=data=journal,commit=5,noatime,barrier=1,errors=remount-ro' >> /boot/armbianEnv.txt


# 使用sed命令替换verbosity的值
sudo sed -i 's/verbosity=1/verbosity=7/g' "/boot/armbianEnv.txt"

echo "已将 /boot/armbianEnv.txt 中的 verbosity=1 修改为 verbosity=7"
...
# 设置默认主机名
#echo "dsdz-h618" > /etc/hostname
#echo "127.0.1.1 dsdz-h618" >>  /etc/hosts

# 设置 root 密码（加密密码）
#sed -i 's|^root:[^:]*:|root:$y$j9T$FTJBHnn.66sgtMD3.LSp9/$9SirJWC/pYbn.eBE6ZlZIi4UXhtDDw8SEMgVn8fFwG8:|' "/etc/shadow"

    # 确保权限正确（避免开机登录失败）
#    chmod 640 "$TARGET_ROOTFS/etc/shadow"
#    chown root:shadow "$TARGET_ROOTFS/etc/shadow"


# 创建默认用户（例：user: pi，密码: 1234）
#chroot  useradd -m -s /bin/bash dsdz
#echo "dsdz:dsdz" | chpasswd
#echo "dsdz ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/dsdz
...
#rm -f /etc/systemd/system/multi-user.target.wants/armbian-firstlogin.service
